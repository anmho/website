...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !stateFromCookie || state !== stateFromCookie) {
    return NextResponse.json({ error: 'Invalid OAuth callback state or code.' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to exchange Spotify code for tokens.' }, { status: 500 });
  }
}
