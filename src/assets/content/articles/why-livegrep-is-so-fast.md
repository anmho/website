...
Livegrep feels instantaneous because it does almost everything *before* you type a query. It builds a reusable index file, memory-maps it, and serves searches from that index rather than rescanning the original repositories on every keystroke. See the [Livegrep README](https://github.com/livegrep/livegrep) for details on indexing and memory-mapping.
...
Livegrep is a two-part system and is partially inspired by Google Code Search. ([Livegrep README](https://github.com/livegrep/livegrep))
...
Because the backend works from a prebuilt index (optionally saved to disk), it doesn’t need to touch the original repos during queries. The index is memory-mapped so it can be larger than RAM but still queried quickly; performance improves if the index fits in memory. The index file is typically **3–5× the size of the indexed text**. ([Livegrep README](https://github.com/livegrep/livegrep))

Livegrep uses Google’s **RE2** regex engine, which is fast and safe (no catastrophic backtracking), but not fully PCRE-compatible. ([Livegrep README](https://github.com/livegrep/livegrep))
...
The `codesearch` backend can build an in-memory index or write an index file to disk. Once built, that index is standalone; you can run the search server by loading the index file with no direct access to the repos. ([Livegrep README](https://github.com/livegrep/livegrep))
...
GitHub Code Search is a **hosted** experience across multiple repositories with rich query features. It supports **regular expressions, boolean operations, and query operators**, and is always up to date. ([GitHub Code Search](https://github.com/features/code-search))
...
Google’s `codesearch` is a **command-line tool** for indexing and running regex searches over large code trees. It is a set of CLI programs that index and search source code. ([Google Code Search](https://github.com/google/codesearch))
...
grep.app is a hosted code search service that searches across **a million GitHub repositories**. ([grep.app](https://grep.app/))
